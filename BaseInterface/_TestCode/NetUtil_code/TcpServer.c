...
int TcpSer_InitSoc(StcTcpServer *pStcTcpServer)
{
	...
	setsockopt(pStcTcpServer->m_TcpSerSok, SOL_SOCKET, SO_REUSEADDR, &buflen,
			len);
...
//	nbuflen = len;
	send_ret = send(sockfd, buf, nbuflen, MSG_NOSIGNAL);
	if (send_ret == -1)
	{
	...
	while (send_ret < nbuflen)
	{
		pos += send_ret;
		nbuflen -= send_ret;
		send_ret = send(sockfd, &buf[pos], nbuflen, MSG_NOSIGNAL);
