...
int TcpClt_LinkSer(StcTcpClient *p_TcpClient, char p_acIp[], int p_nPort)
{
	...
	TcpSvr_addr.sin_addr.s_addr = inet_addr(p_acIp);
...
int TcpClt_Send(StcTcpClient *p_TcpClient, char p_abuf[], int p_nBufSize)
{
	...
	//p_nBufSize = len;
	if ((send_ret = send(p_TcpClient->m_TcpClnSok, p_abuf, p_nBufSize,0)) == -1)
	{
		ret = -1;
		return -1;
	}
	while (send_ret < p_nBufSize)
	{
		pos += send_ret;
		p_nBufSize -= send_ret;
		send_ret = send(p_TcpClient->m_TcpClnSok, &p_abuf[pos], p_nBufSize, 0);
